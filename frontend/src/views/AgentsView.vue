...
<template>
  <div class="max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-10">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
      <div>
        <h1 class="text-xl md:text-2xl font-semibold text-[var(--color-text)]">Agent Management</h1>
        <p class="text-sm text-[var(--color-text-muted)] mt-1">Create and customize AI simulation agents</p>
      </div>
      <button
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-6 mt-6">
      <div class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg px-4 py-3">
        <div class="text-xs text-[var(--color-text-muted)]">My Agents</div>
        <div class="text-lg font-semibold text-[var(--color-text)]">{{ customCount }}</div>
      </div>
      <div class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg px-4 py-3">
        <div class="text-xs text-[var(--color-text-muted)]">Templates</div>
        <div class="text-lg font-semibold text-[var(--color-text)]">{{ templateCount }}</div>
      </div>
      <div class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg px-4 py-3">
        <div class="text-xs text-[var(--color-text-muted)]">Departments</div>
    ...
    <div v-if="filteredAgents.length === 0 && !searchQuery && filterDepartment === 'all' && filterSection === 'custom'" class="text-center py-16 md:py-24">
      ...
      <h2 class="text-base font-semibold text-[var(--color-text)] mb-2">No custom agents yet</h2>
      <p class="text-sm text-[var(--color-text-secondary)] mb-6 max-w-sm mx-auto">
        Create your first agent or start from a template to customize simulation personas.
      </p>
      <button
    ...
    <div v-else-if="filteredAgents.length === 0" class="text-center py-12">
      <p class="text-sm text-[var(--color-text-muted)]">No agents match your filters.</p>
      <button
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      >
        ...
        <div class="flex items-start gap-3 mb-3">
          ...
          <div class="flex items-center gap-1 ml-1 shrink-0">
            <button
              ...
              :title="agent._section === 'templates' ? 'Use template' : 'Clone agent'"
            >
        ...
        <div class="flex items-center gap-2 mb-3">
          <span class="text-[10px] font-medium text-[var(--color-text-muted)] uppercase tracking-wider">Personality</span>
