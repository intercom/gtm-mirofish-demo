...
class TestListScenarios:
    ...
    def test_returns_all_known_scenarios(self, client):
        scenarios = client.get(f"{PREFIX}/scenarios").get_json()["scenarios"]
...
class TestGetScenario:
    ...
    def test_returns_200_for_known_scenarios(self, client, scenario_id):
        resp = client.get(f"{PREFIX}/scenarios/{scenario_id}")
        assert resp.status_code == 200

    @pytest.mark.parametrize("scenario_id", KNOWN_SCENARIO_IDS)
    def test_full_scenario_has_required_fields(self, client, scenario_id):
        data = client.get(f"{PREFIX}/scenarios/{scenario_id}").get_json()
    ...
    def test_returns_404_for_unknown_scenario(self, client):
        resp = client.get(f"{PREFIX}/scenarios/nonexistent_scenario")
        assert resp.status_code == 404
        assert "error" in resp.get_json()

    def test_id_matches_request(self, client):
        for sid in KNOWN_SCENARIO_IDS:
            data = client.get(f"{PREFIX}/scenarios/{sid}").get_json()
            assert data["id"] == sid
...
class TestGetSeedData:
    ...
    def test_returns_200_for_known_types(self, client, data_type):
        resp = client.get(f"{PREFIX}/seed-data/{data_type}")
        assert resp.status_code == 200

    @pytest.mark.parametrize("data_type", KNOWN_SEED_TYPES)
    def test_response_is_valid_json_object(self, client, data_type):
        data = client.get(f"{PREFIX}/seed-data/{data_type}").get_json()
    ...
    def test_account_profile_fields(self, client):
        profile = client.get(f"{PREFIX}/seed-data/account_profiles").get_json()["profiles"][0]
    ...
    def test_persona_templates_has_personas_list(self, client):
        data = client.get(f"{PREFIX}/seed-data/persona_templates").get_json()
        assert "personas" in data
        assert isinstance(data["personas"], list)
        assert len(data["personas"]) > 0

    def test_signal_definitions_has_signals_list(self, client):
        data = client.get(f"{PREFIX}/seed-data/signal_definitions").get_json()
        assert "signals" in data
        assert isinstance(data["signals"], list)
        assert len(data["signals"]) > 0

    def test_returns_404_for_unknown_type(self, client):
        resp = client.get(f"{PREFIX}/seed-data/nonexistent_data")
        assert resp.status_code == 404
        assert "error" in resp.get_json()
...
class TestGetScenarioSeedText:
    @pytest.mark.parametrize("scenario_id", KNOWN_SCENARIO_IDS)
    def test_returns_200_for_known_scenarios(self, client, scenario_id):
        resp = client.get(f"{PREFIX}/scenarios/{scenario_id}/seed-text")
        assert resp.status_code == 200

    @pytest.mark.parametrize("scenario_id", KNOWN_SCENARIO_IDS)
    def test_response_contains_seed_text(self, client, scenario_id):
        data = client.get(f"{PREFIX}/scenarios/{scenario_id}/seed-text").get_json()
    ...
    def test_seed_text_matches_full_scenario(self, client):
        ...
        full = client.get(f"{PREFIX}/scenarios/outbound_campaign").get_json()
        seed = client.get(f"{PREFIX}/scenarios/outbound_campaign/seed-text").get_json()
        assert seed["seed_text"] == full["seed_text"]

    def test_returns_404_for_unknown_scenario(self, client):
        resp = client.get(f"{PREFIX}/scenarios/nonexistent_scenario/seed-text")
        assert resp.status_code == 404
        assert "error" in resp.get_json()
...
class TestSimulateValidation:
    ...
    def test_rejects_blank_seed_text(self, client):
        resp = client.post(f"{PREFIX}/simulate", json={"seed_text": "   "})
        assert resp.status_code == 400
        data = resp.get_json()
        assert data["success"] is False
    ...
    def test_rejects_missing_content_type(self, client):
        ...
        resp = client.post(f"{PREFIX}/simulate", data="not json")
        assert resp.status_code >= 400
        data = resp.get_json()
        assert data["success"] is False
