...
# ---------------------------------------------------------------------------
# Simple What-If API — module-level functions used by API endpoints
# and sensitivity_analyzer. Complements the class-based WhatIfEngine above.
# ---------------------------------------------------------------------------
...
_whatif_variants: Dict[str, List[str]] = {}
...
def create_whatif_variant(
    base_simulation_id: str,
    modifications: List[Dict[str, Any]],
) -> Dict[str, Any]:
    """Create and 'run' a what-if scenario variant (simple API)."""
    ...
    deltas = {}
    for metric in _SIMPLE_OUTCOME_METRICS:
    ...
    logger.info("Created what-if variant %s for base %s", variant_id, base_simulation_id)
    return scenario


def list_whatif_variants(base_simulation_id: str) -> List[Dict[str, Any]]:
...
def get_whatif_variant(variant_id: str) -> Optional[Dict[str, Any]]:
