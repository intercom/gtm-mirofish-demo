...
const endpoints = [
  // Health
  { category: 'Health', method: 'GET', path: '/health', description: 'Basic health check', request: null, response: '{"status":"ok","service":"MiroFish Backend"}' },

  // Scenarios
  { category: 'Scenarios', method: 'GET', path: '/api/gtm/scenarios', description: 'List all GTM scenario templates', request: null, response: '{"scenarios":[{"id":"...","name":"...","description":"...","category":"...","icon":"..."}]}' },
  { category: 'Scenarios', method: 'GET', path: '/api/gtm/scenarios/:scenario_id', description: 'Get a scenario template with full configuration', request: null, response: '{"scenario":{...},"seed_data":{...}}' },
  { category: 'Scenarios', method: 'GET', path: '/api/gtm/scenarios/:scenario_id/seed-text', description: 'Get seed text for a specific scenario', request: null, response: '{"seed_text":"..."}' },
  { category: 'Scenarios', method: 'GET', path: '/api/gtm/seed-data/:data_type', description: 'Get seed data by type (personas, industries, etc.)', request: null, response: '{"data":[...]}' },
  { category: 'Scenarios', method: 'POST', path: '/api/gtm/simulate', description: 'Start a unified GTM simulation from scenario config', request: [
  ...
  // Graph
  { category: 'Graph', method: 'GET', path: '/api/graph/project/list', description: 'List all projects', request: null, response: '{"success":true,"data":[...],"count":0}' },
  { category: 'Graph', method: 'GET', path: '/api/graph/project/:project_id', description: 'Get project details', request: null, response: '{"success":true,"data":{...}}' },
  { category: 'Graph', method: 'DELETE', path: '/api/graph/project/:project_id', description: 'Delete a project', request: null, response: '{"success":true,"message":"..."}' },
  { category: 'Graph', method: 'POST', path: '/api/graph/project/:project_id/reset', description: 'Reset project state for graph rebuild', request: null, response: '{"success":true,"data":{...}}' },
  { category: 'Graph', method: 'POST', path: '/api/graph/ontology/generate', description: 'Upload files and generate ontology definition (multipart/form-data)', request: [
  ...
  { category: 'Graph', method: 'POST', path: '/api/graph/build', description: 'Build knowledge graph from project ontology (async)', request: [
  ...
  { category: 'Graph', method: 'GET', path: '/api/graph/task/:task_id', description: 'Check async task status', request: null, response: '{"success":true,"data":{"task_id":"...","status":"...","progress":0}}' },
  { category: 'Graph', method: 'GET', path: '/api/graph/tasks', description: 'List all tasks', request: null, response: '{"success":true,"data":[...],"count":0}' },
  { category: 'Graph', method: 'GET', path: '/api/graph/data/:graph_id', description: 'Get graph nodes and edges', request: null, response: '{"success":true,"data":{...}}' },
  { category: 'Graph', method: 'DELETE', path: '/api/graph/delete/:graph_id', description: 'Delete a Zep graph', request: null, response: '{"success":true,"message":"..."}' },
  ...
  // Chat
  { category: 'Chat', method: 'POST', path: '/api/report/chat', description: 'Chat with the simulation world using report context', request: [
...
const categories = ['Health', 'Scenarios', 'Graph', 'Simulation', 'Report', 'Chat', 'Settings']
...
const groupedEndpoints = computed(() => {
  ...
  for (const cat of categories) {
    const items = filteredEndpoints.value.filter((e) => e.category === cat)
...
async function tryIt() {
  ...
  try {
    let path = resolvedPath(ep)
    // For /health, it's not under /api so use a raw fetch
    if (path === '/health') {
    ...
    // Strip /api prefix since the client already has baseURL=/api
  ...
  <div class="flex h-[calc(100vh-56px)] overflow-hidden">
    ...
    <aside class="w-72 shrink-0 border-r border-[var(--color-border)] bg-[var(--color-surface)] overflow-y-auto">
      ...
      <nav class="p-2">
        <template v-for="(items, category) in groupedEndpoints" :key="category">
          <div class="px-2 pt-3 pb-1 text-xs font-semibold uppercase tracking-wider text-[var(--color-text-muted)]">
            {{ category }}
          </div>
          <button
            v-for="ep in items"
          ...
          >
            <span
              ...
              :style="{
                backgroundColor: METHOD_COLORS[ep.method].bg,
    ...
    <main class="flex-1 overflow-y-auto p-6 md:p-8">
      <div class="max-w-3xl">
        ...
        <p class="text-sm text-[var(--color-text-secondary)] mb-6">{{ selectedEndpoint.description }}</p>
        ...
        <section class="mb-6">
          ...
          <pre class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-lg px-4 py-3 text-sm font-mono text-[var(--color-text-secondary)] overflow-x-auto whitespace-pre-wrap">{{ JSON.stringify(JSON.parse(selectedEndpoint.response), null, 2) }}</pre>
        </section>
