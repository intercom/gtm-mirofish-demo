...
class TestReportChat:
    def test_chat_keyword_match(self, client):
        resp = client.post("/api/report/chat", json={"message": "Tell me about subject line performance"})
    ...
    def test_chat_default_response(self, client):
        resp = client.post("/api/report/chat", json={"message": "random unmatched query xyz"})
        assert resp.status_code == 200
        data = resp.get_json()["data"]
        assert "response" in data
    ...
    def test_chat_tool_calls_present(self, client):
        resp = client.post("/api/report/chat", json={"message": "roi analysis"})
        data = resp.get_json()["data"]
        assert "tool_calls" in data
        assert len(data["tool_calls"]) > 0
        assert "name" in data["tool_calls"][0]

    def test_chat_each_keyword(self, client):
        keywords = ["subject line", "persona", "healthcare", "objection", "roi", "cadence", "fin"]
        for kw in keywords:
            resp = client.post("/api/report/chat", json={"message": kw})
...
class TestInterview:
    def test_interview_keyword_fallback(self, client):
    ...
    def test_interview_default_fallback(self, client):
        resp = client.post("/api/simulation/interview", json={
    ...
    def test_interview_batch(self, client):
        resp = client.post("/api/simulation/interview/batch")
        assert resp.status_code == 200

    def test_interview_all(self, client):
        resp = client.post("/api/simulation/interview/all")
        assert resp.status_code == 200

    def test_interview_history(self, client):
        resp = client.post("/api/simulation/interview/history")
        assert resp.status_code == 200


class TestSettings:
    def test_test_llm(self, client):
        resp = client.post("/api/settings/test-llm")
        assert resp.status_code == 200
        data = resp.get_json()
        assert data["ok"] is True
        assert "model" in data

    def test_test_zep(self, client):
        resp = client.post("/api/settings/test-zep")
        assert resp.status_code == 200
...
class TestDemoSpeed:
    def test_get_speed(self, client):
        resp = client.get("/api/demo/speed")
        assert resp.status_code == 200
        assert resp.get_json()["data"]["speed"] == 1.0

    def test_set_speed(self, client):
        resp = client.post("/api/demo/speed", json={"speed": 5.0})
        assert resp.status_code == 200
        assert resp.get_json()["data"]["speed"] == 5.0
        # Verify it persists
        get_resp = client.get("/api/demo/speed")
        assert get_resp.get_json()["data"]["speed"] == 5.0

    def test_speed_minimum_clamped(self, client):
        resp = client.post("/api/demo/speed", json={"speed": 0.01})
        assert resp.status_code == 200
        assert resp.get_json()["data"]["speed"] >= 0.1


class TestDemoSkip:
    def test_skip_graph(self, client):
        client.post("/api/graph/build")
        resp = client.post("/api/demo/skip/graph")
        assert resp.status_code == 200
        assert resp.get_json()["data"]["skipped"] == "graph"

    def test_skip_simulation(self, client):
        client.post("/api/simulation/create")
        resp = client.post("/api/demo/skip/simulation")
        assert resp.status_code == 200
        assert resp.get_json()["data"]["skipped"] == "simulation"

    def test_skip_report(self, client):
        client.post("/api/report/generate", json={"simulation_id": "demo-sim-00001"})
        resp = client.post("/api/demo/skip/report")
        assert resp.status_code == 200
        assert resp.get_json()["data"]["skipped"] == "report"

    def test_skip_unknown_phase(self, client):
        resp = client.post("/api/demo/skip/unknown")
...
class TestDemoReset:
    def test_reset_clears_state(self, client):
        ...
        client.post("/api/simulation/create")
        client.post("/api/report/generate", json={"simulation_id": "demo-sim-00001"})
        client.post("/api/demo/speed", json={"speed": 10.0})
        ...
        speed = client.get("/api/demo/speed").get_json()["data"]["speed"]
        assert speed == 1.0
...
class TestChatWithLLM:
    ...
    def test_chat_uses_llm_when_available(self, mock_llm, client):
        resp = client.post("/api/report/chat", json={"message": "Tell me about the data"})
        assert resp.status_code == 200
        data = resp.get_json()["data"]
    ...
    def test_chat_falls_back_when_llm_unavailable(self, mock_llm, client):
        resp = client.post("/api/report/chat", json={"message": "subject line performance"})
        assert resp.status_code == 200
        data = resp.get_json()["data"]
    ...
    def test_interview_uses_llm_when_available(self, mock_llm, client):
        resp = client.post("/api/simulation/interview", json={
            "agent_name": "Sarah Chen",
            "agent_role": "VP of Support",
