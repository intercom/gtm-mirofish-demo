...
Additionally provides memory configuration endpoints for agent memory
behavior settings, Zep connection testing, and memory usage statistics.

Also exposes the AgentMemory abstraction layer over REST, working in
demo/mock mode (in-memory backend) when no ZEP_API_KEY is configured.
"""
...
# ---------------------------------------------------------------------------
# Memory configuration endpoints (memory_config_bp Blueprint)
# ---------------------------------------------------------------------------

memory_config_bp = Blueprint('memory_config', __name__, url_prefix='/api/v1/memory')
...
@memory_config_bp.route('/config', methods=['GET'])
def get_config():
    """Return current memory configuration."""
    return jsonify(_memory_config)


@memory_config_bp.route('/config', methods=['POST'])
def save_config():
...
@memory_config_bp.route('/stats', methods=['GET'])
def get_stats():
...
@memory_config_bp.route('/test-connection', methods=['POST'])
def test_connection():
...
# ---------------------------------------------------------------------------
# Agent memory abstraction layer (agent_memory_bp Blueprint)
# ---------------------------------------------------------------------------

agent_memory_bp = Blueprint('agent_memory', __name__, url_prefix='/api/v1/agent-memory')
...
@agent_memory_bp.route('/store', methods=['POST'])
def store_message():
...
@agent_memory_bp.route('/history/<agent_id>/<session_id>', methods=['GET'])
...
@agent_memory_bp.route('/search', methods=['POST'])
def search_memory():
...
@agent_memory_bp.route('/facts/<agent_id>', methods=['GET'])
...
@agent_memory_bp.route('/<agent_id>', methods=['DELETE'])
...
@agent_memory_bp.route('/<agent_id>/<session_id>', methods=['DELETE'])
...
@agent_memory_bp.route('/status', methods=['GET'])
def memory_status():
