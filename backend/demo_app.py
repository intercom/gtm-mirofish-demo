...
def sim_network(sim_id):
    """Network analysis: nodes, edges, clusters, adjacency matrix, comm patterns."""
    ...
    max_round = max(1, min(TOTAL_ROUNDS, int(pct / 100 * TOTAL_ROUNDS)))

    selected_round = request.args.get("round", max_round, type=int)
    selected_round = max(1, min(selected_round, max_round))
    ...
    agents = [
        {"id": i, "name": name, "title": title, "company": company}
        for i, (name, title, company) in enumerate(agents_raw)
    ]
    n = len(agents)
    rng = random.Random(selected_round * 137 + 7)

    # Assign clusters by role keywords
    role_cluster = {
        "VP": 0, "Support": 0, "Mgr": 0, "CS": 0,
        "CX": 1, "Dir": 1, "Digital": 1,
        "Head": 2, "Lead": 2, "Ops": 2,
        "IT": 3, "CTO": 3, "CFO": 3, "Director": 3,
    }
    for a in agents:
        a["cluster"] = 0
        for kw, cid in role_cluster.items():
            if kw in a["title"]:
                a["cluster"] = cid
                break

    # Generate edges — prefer intra-cluster connections
    edge_set = set()
    edges = []
    for i in range(n):
        num_conn = rng.randint(2, min(5, n - 1))
        same = [j for j in range(n) if j != i and agents[j]["cluster"] == agents[i]["cluster"]]
        diff = [j for j in range(n) if j != i and agents[j]["cluster"] != agents[i]["cluster"]]
        targets = rng.sample(same, min(max(1, num_conn // 2), len(same)))
        remaining = num_conn - len(targets)
        if remaining > 0 and diff:
            targets += rng.sample(diff, min(remaining, len(diff)))
        for t in targets:
            key = (min(i, t), max(i, t))
            if key not in edge_set:
                ...
                edges.append({
                    ...
                    "type": rng.choice(["reply", "topic_adoption", "sentiment_alignment"]),
                })

    # Centrality from degree
    deg = [0] * n
    for e in edges:
        deg[e["source"]] += 1
        deg[e["target"]] += 1
    ...
    for a in agents:
        a["degree"] = deg[a["id"]]
        a["centrality"] = round(deg[a["id"]] / max_deg, 3)
        a["betweenness"] = round(min(1.0, (deg[a["id"]] / max_deg) * rng.uniform(0.4, 1.1)), 3)

    # Clusters
    cluster_info = [
        ("Support Leaders", "#2068FF"),
        ("CX Strategists", "#ff5600"),
        ("Operations", "#AA00FF"),
        ("Tech & Finance", "#009900"),
    ]
    clusters = [
        {"id": i, "label": label, "color": color,
         "members": [a["id"] for a in agents if a["cluster"] == i]}
        for i, (label, color) in enumerate(cluster_info)
    ]
    ...
    # Communication patterns over time buckets
    comm_rng = random.Random(selected_round * 31 + 13)
    num_buckets = min(24, max(1, selected_round))
    bucket_size = max(1, selected_round // num_buckets)
    comm_patterns = []
    for b in range(num_buckets):
        round_label = min((b + 1) * bucket_size, selected_round)
        for _ in range(comm_rng.randint(3, 8)):
            src = comm_rng.randint(0, n - 1)
            tgt = comm_rng.choice([j for j in range(n) if j != src])
            comm_patterns.append({
                "round": round_label,
                "source": src,
                "target": tgt,
                "count": comm_rng.randint(1, 4),
                "sentiment": round(comm_rng.uniform(-0.3, 1.0), 2),
            })
...
# Network endpoints registered earlier (lines ~352-430) — removed duplicates.
